...
def setup_nest(G, training_set):
    ...
    #citation_neurons = nest.Create("iaf_psc_delta", len(G), {"tau_m": 100000, "V_reset": 0, "V_th": 1, "tau_minus": 30})
    citation_neurons = nest.Create("iaf_psc_alpha", len(G), {"I_e": 376})
    ...
    #        citation_neurons[n].set({"t_ref": 1})
...
def run_simulation(test_node_id, citation_neurons, class_neurons, time_steps=20, reset=False):
    ...
    spike = nest.Create("spike_generator", params={"spike_times": [1.0], "spike_weights": [1e4]})
    ...
    result_table = {}
    for c, neuron in class_neurons.items():
        weight = nest.GetConnections(source=test_neuron, target=neuron).get(["weight"])
        result_table[c] = weight["weight"]

    highest = max(result_table.items(), key=lambda x: x[1])[0]
